    ...
    public class QRCodeEncoder
    {
		...
		public virtual bool[][] calQrcode(byte[] qrcodeData)
		{
			...
			try
			{
                ...
                var memoryStream = new MemoryStream(Resources.ResourceManager.GetObject(fileName) as byte[]);
        ...
        /// <summary>
        /// Encode the content using the encoding scheme given。
        /// 2020-8-30 陈礼轩：经测试，微信7.0.16 和 魅族自带浏览器8.2.1 扫描二维码时中文转成了乱码，可用编码Encoding.Utf8解决
        /// </summary>
        /// <param name="content"></param>
        /// <param name="encoding"></param>
        /// <returns></returns>
